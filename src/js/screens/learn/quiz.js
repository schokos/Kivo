...
function renderQuiz() {
  ...
  el.innerHTML=`
    <div class="quiz-hdr">
      ...
      <div class="quiz-scores"><span style="color:var(--lime)">âœ“ ${qOk}</span> <span style="color:var(--danger)">âœ— ${qNo}</span></div>
    </div>
    ...
    <div style="display:flex;gap:8px"><button class="btn btn-sm" onclick="startQuiz()">â†º Neu</button></div>`;
}
function answerQuiz(chosen,correct,btn) {
  ...
  if(chosen===correct) {
    ...
    document.getElementById('quiz-fb').textContent='âœ“ Richtig!';
    ...
    qNo++; document.getElementById('quiz-fb').textContent='âœ— Richtig: '+correct.replace(/_/g,' ');
...
function endQuiz() {
  ...
  el.innerHTML=`
    <div class="quiz-result">
      ...
      <div style="margin:10px 0;font-size:14px;color:var(--muted)">${qOk} richtig Â· ${qNo} falsch Â· ${qDeck.length} gesamt</div>
      <div style="font-size:13px;margin-bottom:18px;color:var(--lime)">+${qOk*5} XP verdient!</div>
      <button class="btn btn-lime" onclick="startQuiz()">â†º Nochmal</button>
      <button class="btn btn-sm" style="margin-left:8px" onclick="goTo('stats')">ðŸ“Š Statistiken</button>
    </div>`;
  updateHomeStats();
}
